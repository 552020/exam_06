...
// int ids[65536];
// char *msgs[65536];
// char buf_write[42], buf_read[1001]; // Probably I will need to increase
// these.
char buf_write[1100042], buf_read[1000000];
    ...
    for (int fd = 0; fd <= max_fd; fd++) {
      ...
      if (fd == sockfd) {
        ...
        if (bytes_read <= 0) {
          ...
          for (int i = 0, j = strlen(clients[fd].msg); i < bytes_read;
               i++, j++) {
